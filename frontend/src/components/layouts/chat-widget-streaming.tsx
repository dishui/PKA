...
import { Send, X, MessageCircle, Loader2, Languages, Zap } from 'lucide-react';
...
interface ChatWidgetStreamingProps {
  apiUrl?: string;
  title?: string;
  placeholder?: string;
  primaryColor?: string;
...
export const ChatWidgetStreaming: React.FC<ChatWidgetStreamingProps> = ({
  ...
  const handleSendMessageStreaming = async (userInput: string) => {
    setIsLoading(true);
    ...
    try {
      ...
      const eventSource = new EventSource(
        `${apiUrl}/chat/stream?${params.toString()}`
      );
      ...
      eventSource.addEventListener('sources', (event) => {
        console.log(`Found ${event.data} relevant sources`);
      });
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === streamingMessageId
              ? {
                  ...msg,
                  content: streamedContent || 'Sorry, an error occurred. Please try again.',
                  isStreaming: false,
                }
              : msg
          )
  ...
  const handleSendMessageNonStreaming = async (userInput: string) => {
    setIsLoading(true);
  ...
  const handleSendMessage = async () => {
    ...
    if (useStreaming) {
      await handleSendMessageStreaming(userInput);
    } else {
      await handleSendMessageNonStreaming(userInput);
    }
  };
  ...
  const toggleWidget = () => {
    ...
    if (!isOpen && messages.length === 0) {
      const welcomeMessage: Message = {
        id: 'welcome',
        role: 'assistant',
        content: 'Hi! How can I help you today? I can answer questions in multiple languages! 🌍',
        timestamp: new Date(),
      };
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              {useStreaming && (
                <Zap size={14} className="opacity-75" title="Streaming enabled" />
              )}
            </div>
...
export default ChatWidgetStreaming;
