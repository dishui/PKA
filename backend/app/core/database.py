...
async def init_db() -> None:
    ...
    try:
        ...
        from app.models import user, agent, transaction, escrow, risk
...
# Sync engine for Alembic migrations - Optional for development
sync_engine = None
try:
    # Try to create sync engine for migrations
    ...
    logger.info("Sync engine created for migrations")
except ImportError as e:
    logger.warning("Sync engine not available - migrations will need to be run manually", error=str(e))
except Exception as e:
    logger.warning("Failed to create sync engine", error=str(e)) 
